...
```{r bitcoin_graphs}
ggplot(btcAllClean, aes(x=Date, y=Trvou, group = 1)) + geom_line()
ggplot(btcAllClean, aes(x=Date, y=Mkpru, group = 1)) + geom_line() 
ggplot(btcAllClean, aes(x=Date, y=Hrate, group = 1)) + geom_line()
ggplot(btcAllClean, aes(x=Date, y=Diff, group = 1)) + geom_line()
...
ggplot(data = GoldAnalysis, aes(x = Date, y = Value, color=as.factor(Curr))) +
    geom_point(size=0.1) + facet_wrap(~Curr)

```
```{r attr_analysis_spcomposite}
