...
ggplot(data = example, aes(x = Country.Name, y =`Urban population growth (annual %)`, fill=as.factor(Country.Name))) +
    geom_bar(stat = "identity") + coord_flip() + theme_minimal() + labs(fill='Country', x='Country')

ggplot(data = example, aes(x = Country.Name, y = `Trade (% of GDP)`, fill=as.factor(Country.Name))) +
    geom_bar(stat = "identity") + coord_flip() + theme_minimal() + labs(fill='Country', x='Country')

ggplot(data = example, aes(x = Country.Name, y = `Total natural resources rents (% of GDP)`, fill=as.factor(Country.Name))) +
    geom_bar(stat = "identity") + coord_flip() + theme_minimal() + labs(fill='Country', x='Country')

ggplot(data = example, aes(x = Country.Name, y = `Population, total`, fill=as.factor(Country.Name))) +
    geom_bar(stat = "identity") + coord_flip() + theme_minimal() + labs(fill='Country', x='Country')

ggplot(data = example, aes(x = Country.Name, y = `Net primary income (Net income from abroad) (current USD)`, fill=as.factor(Country.Name))) + geom_bar(stat = "identity") + coord_flip() + theme_minimal() + labs(fill='Country', x='Country')

ggplot(data = example, aes(x = Country.Name, y = `Birth rate, crude (per 1,000 people)`, fill=as.factor(Country.Name))) +
    geom_bar(stat = "identity") + coord_flip() + theme_minimal() + labs(fill='Country', x='Country')

ggplot(data = example, aes(x = Country.Name, y = `GDP (current USD)`, fill=as.factor(Country.Name))) +
    geom_bar(stat = "identity") + coord_flip() + theme_minimal() + labs(fill='Country', x='Country')
...
for (country in hdiTop) {
  ...
  # check <- cor(x = countryExample$MeanMkpru, y = countryExample$`GDP per capita (current USD)`, method = "pearson", use = "complete.obs")
  #if (check < 0.6) {next}
