...
```{r bitcoin_graphs}
ggplot(btcAllClean, aes(x=Date, y=Trvou, group = 1)) + geom_line() + transition_reveal(Date)
ggplot(btcAllClean, aes(x=Date, y=Mkpru, group = 1)) + geom_line() + transition_reveal(Date)
ggplot(btcAllClean, aes(x=Date, y=Hrate, group = 1)) + geom_line() + transition_reveal(Date)
ggplot(btcAllClean, aes(x=Date, y=Diff, group = 1)) + geom_line() + transition_reveal(Date)




#btcAllClean <- btcAllClean %>% pivot_longer(cols='Trvou':'Diff', names_to = 'Param', values_to = 'Value')
#ggplot(btcAllClean, aes(x=Date, y=Value, color = as.factor(Param))) + geom_line() + facet_grid(~Param)
```
...
#example2019 <- wdiTop %>% filter(Year == 2019) %>% select(-Year) %>% pivot_longer(cols='Urban population growth (annual %)':'Birth rate, #crude (per 1,000 people)', names_to = 'Indicator', values_to = 'Value')
#ggplot(data = example2019, aes(x = Country.Name, y = Value, fill=as.factor(Country.Name))) + facet_grid(Indicator ~ .) +
